...
class Policy(nn.Module):
    def __init__(self,
        ...
        init_ = lambda m: init(m, init_normc_, lambda x: nn.init.constant_(x, 0),
                               nn.init.calculate_gain(self.activation_function_name))
        ...
        if self.pass_belief_to_policy and self.use_belief_encoder:
            self.belief_encoder = utl.FeatureExtractor(dim_belief, self.args.policy_belief_embedding_dim,
                                                       self.activation_function)
        ...
        if action_space.__class__.__name__ == "Discrete":
            ...
            self.dist = DiagGaussian(hidden_layers[-1], num_outputs, self.args.norm_actions_pre_sampling)
        else:
    ...
    def forward(self, state, latent, belief, task):
        ...
        if self.pass_state_to_policy:
            if self.norm_state:
                state = (state - self.state_rms.mean)
                state[..., self.state_rms.var != 0] /= torch.sqrt(self.state_rms.var[self.state_rms.var != 0] + 1e-8)
        ...
        if self.pass_latent_to_policy:
            if self.norm_latent:
                latent = (latent - self.latent_rms.mean)
                latent[..., self.latent_rms.var != 0] /= torch.sqrt(
                    self.latent_rms.var[self.latent_rms.var != 0] + 1e-8)
        ...
        if self.pass_belief_to_policy:
            if self.norm_belief:
                ...
                belief[..., self.belief_rms.var != 0] /= torch.sqrt(
                    self.belief_rms.var[self.belief_rms.var != 0] + 1e-8)
        ...
        if self.pass_task_to_policy:
            ...
            if self.norm_task:
                ...
                task[..., self.task_rms.var != 0] /= torch.sqrt(self.task_rms.var[self.task_rms.var != 0] + 1e-8)
...
class DiagGaussian(nn.Module):
    def __init__(self, num_inputs, num_outputs, norm_actions_pre_sampling, init_std=1.0):
