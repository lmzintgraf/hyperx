    ...
    def insert(self, prev_state, actions, next_state, rewards, done, task):
        ...
        if done.sum() == self.num_processes:  # check if we can process the entire batch at once
            ...
            if self.max_buffer_size > 0:
                ...
                self.tasks[self.insert_idx:self.insert_idx + self.num_processes] = self.running_tasks.reshape(
                    insert_shape)
                self.trajectory_lens[
                self.insert_idx:self.insert_idx + self.num_processes] = self.curr_timestep.clone()
